            ...
            try
            {
                ...
                else
                {
                    ...
                    if (socket.Connected)
                    {
                        ...
                        bytes = socket.EndReceive(result, out SocketError err);
            ...
            catch (SocketException ex)
            {
                ...
                if (ex.ErrorCode == 10054)
                {
                    if (Server)
                    {
                        ...
                        m_OnClientDisconnected?.Invoke(this, new ConnectionEventArgs(socket.RemoteEndPoint.ToString()));
                    }
                    else
                    {
        ...
        void OnClientConnect(IAsyncResult result)
        {
            ...
            catch (ObjectDisposedException)//Server has left.
            {
                if (workerSocket != null)
                {
                    ...
                    m_OnClientDisconnected?.Invoke(this, new ConnectionEventArgs(workerSocket.RemoteEndPoint.ToString()));
                }
            }
            catch (Exception ex)
            {
                ...
                foreach (var it in tcpIpClients)
                {
                    if (it.Key is Socket s)
                    {
                        if (s.RemoteEndPoint.ToString() == address)
                        {
                            ...
                            s.Close();
                            m_OnClientDisconnected?.Invoke(this, new ConnectionEventArgs(address));
                            break;
                        }
                    }
                }
