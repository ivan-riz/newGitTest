...
namespace Gurux.DLMS.Internal
{
    ...
    class GXAPDU
    {
        ...
        private static void GetInitiateRequest(GXDLMSSettings settings, GXICipher cipher, GXByteBuffer data)
        {
            ...
            bb.SetUInt32((uint)settings.ProposedConformance);
        ...
        private static void GetConformance(uint value, GXDLMSTranslatorStructure xml)
        {
            foreach (uint it in Enum.GetValues(typeof(Conformance)))
            {
                if ((it & value) != 0)
                {
        ...
        private static byte[] GetUserInformation(GXDLMSSettings settings, GXICipher cipher)
        {
            ...
            bb.SetUInt32((uint)settings.NegotiatedConformance);
                                          ...
                                          GXByteBuffer encryptedData)
        {
            ...
            if (encryptedData != null && encryptedData.Size != 0)
            {
                GXByteBuffer tmp2 = new GXByteBuffer((ushort)(2 + encryptedData.Size));
            ...
            data.Set(tmp);
            data.SetUInt8((ushort)(offset + 1), (byte)(data.Size - offset - 2));
        }
    }
}
