...
namespace Gurux.DLMS.Objects
{
    public class GXDLMSIEC14908PhysicalStatus : GXDLMSObject, IGXDLMSBase
    {
        ...
        /// </summary>
        [XmlIgnore()]
        public ushort TransmissionErrors
        {
            get;
            set;
        }
        ...
        /// </summary>
        [XmlIgnore()]
        public ushort MessagesValidated
        {
            get;
            set;
        }
        ...
        void IGXDLMSBase.SetValue(GXDLMSSettings settings, ValueEventArgs e)
        {
            ...
            {
                TransmissionErrors = (ushort)e.Value;
            }
            else if (e.Index == 3)
            {
                TransmitFailure = (ushort)e.Value;
            }
            else if (e.Index == 4)
            {
                TransmitRetries = (ushort)e.Value;
            }
            else if (e.Index == 5)
            {
                ReceiveFull = (ushort)e.Value;
            }
            else if (e.Index == 6)
            {
                LostMessages = (ushort)e.Value;
            }
            else if (e.Index == 7)
            {
                MissedMessages = (ushort)e.Value;
            }
            else if (e.Index == 8)
            {
                Layer2Received = (ushort)e.Value;
            }
            else if (e.Index == 9)
            {
                Layer3Received = (ushort)e.Value;
            }
            else if (e.Index == 10)
            {
                MessagesReceived = (ushort)e.Value;
            }
            else if (e.Index == 11)
            {
                MessagesValidated = (ushort)e.Value;
            }
            else
            {
                e.Error = ErrorCode.ReadWriteDenied;
        ...
        {
            TransmissionErrors = (ushort)reader.ReadElementContentAsInt("TransmissionErrors");
            TransmitFailure = (ushort)reader.ReadElementContentAsInt("TransmitFailure");
            TransmitRetries = (ushort)reader.ReadElementContentAsInt("TransmitRetries");
            ReceiveFull = (ushort)reader.ReadElementContentAsInt("ReceiveFull");
            LostMessages = (ushort)reader.ReadElementContentAsInt("LostMessages");
            MissedMessages = (ushort)reader.ReadElementContentAsInt("MissedMessages");
            Layer2Received = (ushort)reader.ReadElementContentAsInt("Layer2Received");
            Layer3Received = (ushort)reader.ReadElementContentAsInt("Layer3Received");
            MessagesReceived = (ushort)reader.ReadElementContentAsInt("MessagesReceived");
            MessagesValidated = (ushort)reader.ReadElementContentAsInt("MessagesValidated");
        }
