...
namespace Gurux.DLMS.Objects
{
    ...
    public class GXXmlReader : IDisposable
    {
        ...
        public ulong ReadElementContentAsULong(string name)
        {
            return ReadElementContentAsULong(name, 0);
        }

        public ulong ReadElementContentAsULong(string name, ulong defaultValue)
        {
            GetNext();
            if (string.Compare(name, reader.Name, true) == 0)
            {
                ulong ret = Convert.ToUInt64(reader.ReadElementContentAsString());
                return ret;
            }
            return defaultValue;
        }
